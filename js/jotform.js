...
{       // Event simulator
    ...
    setConditionEvents: function(){
        try {
            ...
            $H(JotForm.fieldConditions).each(function(pair){
                ...
                if(!$(field)){ return; }
                if(event == "autocomplete"){ // if event type is autocomplete, listen to blur and keyup events
    ...
    */
    ...
    initSpinnerInputs: function()
    {
        ...
        $$('.form-spinner-input').each(function(item){
          ...
          item.validateSpinnerInputs = function()
          {
            ...
            if( userInput && !numeric.test( userInput ) )
            {
                return JotForm.errored( item, JotForm.texts.numeric );
            }
    ...
    initNumberInputs: function()
    {
        ...
        $$('.form-number-input').each(function(item){
          ...
          item.validateNumberInputs = function()
          {
            ...
            if( item.value && !numeric.test( item.value ) )
            {
                return JotForm.errored( item, JotForm.texts.numeric );
            }
    ...
    validator: function(){
        ...
        $A(JotForm.forms).each(function(form){ // for each JotForm form on the page 
            ...
            form.observe('submit', function(e){ // Set on submit validation
                ...
                //for autofill if any in the form, this will clear any autofill data
                //in the local storage when the user submit the form
                if( form.readAttribute('data-autofill') )
                {
                  form.releaseAutoFill();
                }
            });
    ...
    autoFillInitialize: function( params )
    {
      ...
      //function to release the autoFilled data when submitted
      form.releaseAutoFill = function()
      {
          console.log('submitted');
          jQuery.autoFill().stopSavingData();
          jQuery.autoFill().manuallyReleaseData();
      };
      ...
      var _conflicts = {
        ...
        */
        ...
        {
          if( $$('.form-grading-input').length > 0 )
          {
            var total = 0, id = null;
